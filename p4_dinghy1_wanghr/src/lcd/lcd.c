...
void LCD_DrawSpot(u16 x0,u16 y0,u8 r,u16 color){
	int a,b;
	//int di;
	a=0;b=r;
	while (a<=b)
	{
		LCD_DrawCircle(x0,y0,a,color); //0
		a++;
	}
...
{         
    ...
    {       
        ...
        LCD_ShowChar(x,y,*p,1,color); // overlapping mode
        x+=8;
        p++;
    }  
}
...
void LCD_ShowPic(u16 x1,u16 y1,u16 x2,u16 y2,const u8* img)
{
	LCD_Address_Set(x1,y1,x2,y2);
...
void LCD_ShowObject(int x, int y, int obj){
	int x1=x*8,y1=y*8,x2=x*8+8,y2=y*8+8;
	unsigned char *img;
	if (obj==0){
		img = player;
	}
